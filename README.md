...
- 在 `day8` 的业务方法基础上，把查询、添加、删除和库存扣减整合到一个循环菜单中。
- 程序启动后初始化两条示例商品，并通过菜单反复接收用户选择。
- 菜单提供查询商品、添加商品、删除商品、扣减库存和退出五个选项。
- 运行方式：使用 Visual Studio 打开 `day9.sln`，按 F5 运行。
...
- day8 每次运行只执行一次商品删除操作，然后程序结束。
- day9 使用 `while` 循环和菜单选择，把查询、添加、删除、库存扣减整合到同一个程序中。
- 用户选择 `5` 时退出循环，形成持续可操作的商品管理入口。

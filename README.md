...
- 在 `day9` 菜单的基础上增加 `q` 取消/退出操作。
- 所有需要输入数字的步骤都改用 `int.TryParse` 或 `decimal.TryParse`，避免非法输入导致程序崩溃。
- `ProductService` 新增 `IsCancel` 方法，用于统一判断用户是否输入了 `q`。
- 运行方式：使用 Visual Studio 打开 `day10.sln`，按 F5 运行。
...
- day9 主要完成菜单和业务方法的整合，但输入非法内容时可能直接报错。
- day10 增加 `q` 取消当前操作或退出菜单的统一处理。
- day10 使用 `TryParse` 校验数字输入，并在输入错误时提示用户重新操作。
